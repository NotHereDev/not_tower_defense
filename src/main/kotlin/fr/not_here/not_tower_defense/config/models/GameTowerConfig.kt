...
data class GameTowerConfig(
  ...
  fun check() {
    ...
    if(entityType !in EntityType.values().map { it.name }) {
      throw IllegalArgumentException("Invalid entityType $entityType, on config path: towers[?].entityType, for tower $name")
    }
    if(particleType !in Particle.values().map { it.name }) {
      throw IllegalArgumentException("Invalid particleType $particleType, on config path: towers[?].particleType, for tower $name")
    }
    if(towerType !in TowerType.values().map { it.name }) {
      throw IllegalArgumentException("Invalid towerType $towerType, on config path: towers[?].towerType, for tower $name")
    }
    ...
    if(displayItem !in Material.values().map { it.name }) {
      throw IllegalArgumentException("Invalid displayItem $displayItem, on config path: towers[?].displayItem, for tower $name")
    }
  }
}
